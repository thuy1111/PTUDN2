...
include_once("../../controller/cLuuGiaoDichTT.php");
include_once("../../controller/cBenhNhan.php"); // Include the controller for updating appointment status

$pgg = new cGiaoDich();
$lichKham = new cBenhNhan(); // Create an instance of the appointment controller
...
if (isset($_GET['vnp_Amount'])) {
    ...
    if ($vnp_ResponseCode == "00") {
        $result = $pgg->luuGiaoDichThanhToan(
            $vnp_Amount, $vnp_BankCode, $vnp_BankTranNo, $vnp_CardType, 
            $vnp_OrderInfo, $vnp_PayDate, $vnp_ResponseCode, $vnp_TmnCode, 
            $vnp_TransactionNo, $vnp_TransactionStatus, $vnp_TxnRef.rand(0,9999), $vnp_SecureHash
        );

        if ($result) {
            // Update the appointment status to 'paid'
            $updateStatus = $lichKham->capNhatTrangThaiLichKham($maBN, $vnp_TxnRef);
            if ($updateStatus) {
                echo "<script>alert('Thanh toán thành công và trạng thái lịch khám đã được cập nhật!');window.location.href = 'xemlichkham.php';</script>";
            } else {
                echo "<script>alert('Thanh toán thành công nhưng cập nhật trạng thái lịch khám thất bại!');window.location.href = 'xemlichkham.php';</script>";
                exit();
            }        
        }else{
            echo "<script>alert('Không lưu được hóa đơn');</script>";
            exit();
        }
    } else {
        echo "<script>alert('Thanh toán thất bại!');window.location.href = 'xemlichkham.php';</script>";
        exit();
    }
}
?>
...
<main style="height: 1000px;">
        <a href="../../view/benhNhan/xemlichkham.php" class="btn btn-primary">Xem trang danh sách lịch khám</a>
